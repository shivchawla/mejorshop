...
class DetailOrder extends React.Component {
  ...
  renderBasic = () => {
    ...
    const title = `Código de Pedido: #${order.number}`;
    return (
      <ContainerView title={title}>
        <Text colorSecondary style={styleOrder.text}>
          Pedido: {strDate(order.date_created)}
        </Text>
        <Text colorSecondary style={styleOrder.text}>
          Estatus: <Text style={[styleOrder.text, { color: status.color }]}>{status.text}</Text>
        </Text>
      </ContainerView>
    );
  };
  ...
  renderListProduct = () => {
    ...
    return (
      <ContainerView title="Información del Pedido">
  ...
  renderNote = () => {
    const { order } = this.state;
    return (
      <ContainerView title="Nota de Pedido">
      ...
      <ContainerView style={styles.contentFooter}>
        <View style={styles.viewTotal}>
          <View style={styles.totalLeft}>
            ...
            {isTax && (
              <Text h6 colorThird>
                Impuesto {currencyFormatter(order.total_tax, order.currency)}
              </Text>
            )}
            {isShipping && <Text h6 colorThird>
              Envío {currencyFormatter(order.shipping_total, order.currency)}
            </Text>}
            {isDiscount && <Text h6 colorThird>
              Descuento {currencyFormatter(order.discount_total, order.currency)}
            </Text>}
          </View>
          <Text h3 medium>
    ...
    return (
      <ThemedView isFullView>
        ...
        <ScrollView>
          ...
          {order.payment_method_title && order.payment_method_title != '' ? <ContainerView title="Payment method" subTitle={order.payment_method_title} /> : null}
          <AddressInfo
          ...
          {order.customer_note && order.customer_note !='' ? <ContainerView title="Nota de Pedido">
