...
function* createOrderSaga() {
  try {
    ...
    const prepareData = data
      ...
      .update('shipping_lines', packages => {
        return packages.map(pkg => {
          const method = pkg.get('available_method');
          if (method) {
            let total = method.get('method_cost');
            ...
            return Map({
              ...
              method_title: method.get('method_title'),
            });
          }
        }).filter(item => item);
      })
      ...
      .update('meta_data', meta_data => {
        const [ ...keys ] = meta_data.keys();
        return keys.map(key => {
            return Map({key, value: meta_data.get(key)});
        })
      })
      .toJS();
